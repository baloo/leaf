...
static inline char *
rta_getattr_str(struct rtattr *rta)
{
	return (char *)RTA_DATA(rta);
}

static inline __u8
rta_getattr_u8(struct rtattr *rta)
{
	return *(__u8 *)RTA_DATA(rta);
}

static int
parse_rtattr_flags(struct rtattr *tb[], int max, struct rtattr *rta, int len,
...
leaf_netlink_notify_link(struct nl_msg *msg, void *arg)
{
	...
	msg_hdr = nlmsg_hdr(msg);
	{
		...
		case RTM_DELLINK: {
			...
			int msg_len = nlmsg_datalen(msg_hdr);
			struct rtattr *rta_tb[IFLA_MAX + 1];
			const char *ifname = "";

			parse_rtattr_flags(rta_tb, IFLA_MAX, IFLA_RTA(ifi),
					   msg_len, 0);

			if (ifi->ifi_type != ARPHRD_ETHER)
				return -1; /* This is not ethernet and we don't
					      care about */
			if (rta_tb[IFLA_IFNAME] == NULL)
				return -2; /* There is no name, we can't proceed
					      */
			if (rta_tb[IFLA_OPERSTATE] == NULL)
				return -2; /* There is no state, we can't
					      proceed */
